...
describe("createTrip server action", () => {
  ...
  it("should create trip and redirect on success", async () => {
    ...
    const fd = new FormData();
    fd.set("title", "Japan");
    fd.set("description", "Spring");
    fd.set("startDate", "2026-03-01");
    fd.set("endDate", "2026-03-15");
...
describe("addLocation server action", () => {
  ...
  it("should throw when not authenticated", async () => {
    mockAuth.mockResolvedValueOnce(null);
    const fd = new FormData();
    fd.set("address", "Tokyo");
    await expect(addLocation(fd, "trip-123")).rejects.toThrow(
      "Not authenticated",
    );
  });
...
describe("reorderItinerary server action", () => {
  ...
  it("should throw when not authenticated", async () => {
    mockAuth.mockResolvedValueOnce(null);
    await expect(
      reorderItinerary("trip-123", ["loc-1", "loc-2"]),
