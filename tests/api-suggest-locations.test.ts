...
describe("POST /api/ai/suggest-locations", () => {
  beforeEach(() => { mockAuth.mockReset(); mockFindFirst.mockReset(); mockChat.mockReset(); });
  ...
  it("should return array of suggested locations", async () => {
    const fakeTrip = { id: "trip-123", title: "Europe Tour", description: "Backpacking",
    ...
    const fakeAI = JSON.stringify([
      { name: "Barcelona", address: "Barcelona, Spain", reason: "Great food", bestFor: "food", estimatedDays: 2 },
      { name: "Amsterdam", address: "Amsterdam, Netherlands", reason: "Canals", bestFor: "culture", estimatedDays: 3 },
    ]);
    mockAuth.mockResolvedValueOnce({ user: { id: "user-1" } });
  ...
  it("should include existing locations in the AI prompt", async () => {
    const fakeTrip = { id: "trip-123", title: "Europe Tour", description: "Backpacking",
      startDate: new Date("2026-06-01"), endDate: new Date("2026-06-30"),
      locations: [{ id: "loc-1", locationTitle: "Paris", order: 0 }, { id: "loc-2", locationTitle: "Berlin", order: 1 }] };
    mockAuth.mockResolvedValueOnce({ user: { id: "user-1" } });
    mockFindFirst.mockResolvedValueOnce(fakeTrip);
    mockChat.mockResolvedValueOnce(JSON.stringify([]));
    await POST(createMockNextRequest({ tripId: "trip-123" }));
    expect(mockChat).toHaveBeenCalledWith(
      expect.arrayContaining([expect.objectContaining({ role: "user", content: expect.stringContaining("Paris, Berlin") })]),
      expect.anything()
    );
  });
